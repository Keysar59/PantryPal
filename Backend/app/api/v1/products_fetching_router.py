...
def get_product_by_barcode(barcode: str, testing_service: ProductsFetchingService = Depends(get_products_fetching_service)):
    product = testing_service.get_product_by_barcode(barcode)
    return product_to_json(product)

@router.get("/get_product_by_name")
def get_product_by_name(name: str, page: int = 1, testing_service: TestingService = Depends(get_products_fetching_service)):
...
def get_product_by_category(category: str, page: int = 1, testing_service: TestingService = Depends(get_products_fetching_service)):
